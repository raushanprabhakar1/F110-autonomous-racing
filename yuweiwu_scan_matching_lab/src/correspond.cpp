...
void getCorrespondence(vector<Point>& old_points, vector<Point>& trans_points, vector<Point>& points,
  ...
  for(int i = 0; i<n; ++i){
    ...
    while (!(up_stopped && down_stopped)) {
      ...
      if (now_up) {
        ...
        if (up > start_idx) {
          ...
          // int temp_up = (old_points[up].r < p_i_w.r) ? jump_table[up][UP_BIG] : jump_table[up][UP_SMALL];
          // if (temp_up==m || temp_up==-1) {
          //   continue;
          // }
          // else {
          //   up = temp_up;
      ...
      if (!now_up) {
        ...
        if (down < start_idx) {
          ...
          // int temp_down = (old_points[down].r < p_i_w.r) ? jump_table[down][DOWN_BIG] : jump_table[down][DOWN_SMALL];
          // if (temp_down==m || temp_down==-1) {
          //   continue;
          // }
          // else {
          //   down = temp_down;
    ...
    last_best = best;
    // std::cout << "best up"<<best_up << '\n';
    // std::cout << "best down"<<best_down << '\n';
